...
class NextoryDownload:
    ...
    def download_book_parts(self, crypt_iv_base64, crypt_key_base64, spine_urls, identifier):
        ...
        for result in results:
            file_name = f"{identifier}_{result['index']}.epub"
            with open(file_name, 'wb') as f:
                f.write(result['decrypted_data'])
            book_parts_list.append(file_name)
        return book_parts_list

    def delete_parts(self, book_parts_list):
        for file_name in book_parts_list:
            os.remove(file_name)
    ...
    def get_upload_book_epub(self, row):
        ...
        if status == "FALSE":
            try:
                print('Getting Parts:', identifier)
                ...
                if len(spine_urls) < 100:
                    book_parts_names = self.download_book_parts(crypt_iv_base64, crypt_key_base64, spine_urls,
                                                                identifier)
                    print('Merging Parts:', identifier)

                    merge_file_name = f"{identifier}.epub"
                    doMerge(outputio=merge_file_name, files=book_parts_names)

                    with open(merge_file_name, 'rb') as data:
                        s3.upload_fileobj(data, Bucket=self.BUCKET_NAME, Key=f"ebooks/{identifier}.epub")

                    book_parts_names.append(merge_file_name)
                    self.delete_parts(book_parts_names)

                    print(f"File uploaded: '{identifier}'")
    ...
    def run(self):
        ...
        start_index = 100000
        all_rows = self.get_rows(start_row=start_index, count=total)
        chunk = 100

        for start in range(0, total, chunk):
            with multiprocessing.Pool(processes=3, initializer=self.initializer) as pool:
                rows = all_rows[start: start + chunk]
                try:
                    async_result = pool.map_async(self.get_upload_book_epub, rows)
                    updates = async_result.get()
                    g_updates = []
                    ...
                    gc.collect()
                    items = os.listdir()
                    items = [item for item in items if ".epub" in item]
                    self.delete_parts(items)
                    print('At the end of first part. Dir:', os.listdir())
